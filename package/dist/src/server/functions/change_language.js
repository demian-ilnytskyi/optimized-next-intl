...
export default async function changeLagnuage(value, componentIsSWitcher) {
    try {
        ...
        if (locale === value && componentIsSWitcher === true) {
            if (locale === config.defaultLocale) {
                localeValue = config.locales.find((locale) => locale !== config.defaultLocale) ?? config.defaultLocale;
            }
            else {
